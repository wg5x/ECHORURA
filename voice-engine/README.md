...
- [豆包实时语音落地方案](docs/doubao-realtime-voice-plan.md)：基于火山/豆包官方实时语音样例，拆解 S2S、CustomLLM 和意图识别的接入路径。
...
- [S2S 实现](src/README.md)：FastAPI + React 直接接火山/豆包端到端实时语音 S2S。
- [mobile-sdk 工作副本](mobile-sdk/README.md)：从 `ai-engine` 复制出的 Android/Web/Python 联调工作副本，已经拆出可独立构建的 Android `:sdk` module 和 demo `:app`。
...
- VAD / 打断暂不纳入当前实现；等技术方案和可测试方案明确后再作为独立迭代。
- 下一阶段再做意图识别和业务路由，把稳定的对话输入转成 `route_decision`、`scenario_command`、`native_action` 等结构化事件。
