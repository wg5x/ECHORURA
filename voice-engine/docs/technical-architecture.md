...
- ASR：把用户语音转文本。
- TTS：把模型回复转语音。
- VAD / 打断：用户插话时停止当前播报或取消当前 turn。
- 音频 3A：降噪、回声消除、自动增益等基础音频处理。
- RTC：第一阶段优先参考火山 `rtc-aigc-demo` / RTC 对话式 AI，复用其低延迟音频链路、ASR、TTS、VAD、打断和音频处理能力。
- S2S：第二阶段评估豆包端到端实时语音模型，用于自然语音体验，不直接承担强业务动作执行。

边界：

- 实时语音层只负责“听见、转写、播报、打断、同步状态”。
- 意图识别、二次确认、业务路由和执行权限仍由 Voice Gateway / Semantic Router 控制。
...
- 统一接收语音转写文本。
- 承接火山 RTC 对话式 AI 的 CustomLLM 回调，或承接 H5 / Native Shell 直接上传的转写文本。
